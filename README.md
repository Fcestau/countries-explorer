...
| **Web i18n (EN/ES)** — optional plus | Implemented as a plus: web i18n as a minimalistic client-side implementation. Translation files live in `shared/` as the single source of truth; i18n *configuration* is platform-specific (AsyncStorage + `expo-localization` on mobile, `localStorage` + `navigator.language` on web). No localized routes, no middleware, no `next-i18next`, no SSR of translations — everything resolves client-side after mount, consistent with the app's client-side data fetching. |
